...
def list_profiles(config):
    ...
    data = []
    profiles = config.data["profile"]

    for profile in profiles:
        p = config.data["profile"][profile]
...
def remove_profile(ctx, config, profile):
    ...
    for p in profile:
        if p not in config.data["profile"]:
            secho(
                f"Profile '{p}' doesn't exist. Can't remove it.", fg="red")
        else:
            del config.data["profile"][p]
            echo(f"Profile '{p}' removed from config")

    config.write_config()
    echo("Changes were successful saved to config file.")
